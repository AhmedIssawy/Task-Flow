...
export const validateDepartmentCreation = (data) => {
  ...
  const allowedFields = [
    "name", "phone", "email", "location", "description", "establishedYear", "collegeId", "universityId"
  ];
...
export const validateDepartmentUpdate = (data) => {
  ...
  const allowedFields = [
    "name", "phone", "email", "location", "description", "establishedYear", "collegeId", "universityId", "courses"
  ];
  const filteredData = pickAllowedFields(data, allowedFields);
  return formatValidationResult(errors, filteredData);
};
