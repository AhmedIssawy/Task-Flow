...
router
  ...
  .get(validateUniversityId, authenticate, getStudentsPageOfUniversity);
...
router
  ...
  .get(getCollegesPage)
  .post(authenticate, authorize(["admin", "super-admin"]), validateUniversityId, createCollege);

router
  .route("/:universityId/colleges/:collegeId")
  .get(getCollegeById)
  .post(authenticate, validateUniversityId, updateCollege);
