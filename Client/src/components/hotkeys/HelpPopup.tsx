...
export const HelpPopup: React.FC = () => {
  ...
  const formatHotkeyVisual = (key: string): string => {
    return key
      .replace('ctrl+', 'Ctrl + ')
      .replace('cmd+', 'Cmd + ')
      .replace('shift+', 'Shift + ')
  ...
  const allHotkeys = useMemo((): HotkeyItem[] => {
    ...
    hotkeys.push(
      ...
      {
        key: HOTKEY_KEYS.SIDEBAR_TOGGLE,
        description: HOTKEY_DESCRIPTIONS[HOTKEY_KEYS.SIDEBAR_TOGGLE],
        category: 'navigation',
        visual: formatHotkeyVisual(HOTKEY_KEYS.SIDEBAR_TOGGLE)
      }
    );

    // Table navigation (only for roles that have tables)
    if (['admin', 'super-admin', 'teacher'].includes(role || '')) {
      hotkeys.push(
        {
          key: HOTKEY_KEYS.TABLE_UP,
          description: HOTKEY_DESCRIPTIONS[HOTKEY_KEYS.TABLE_UP],
          category: 'table',
          visual: formatHotkeyVisual(HOTKEY_KEYS.TABLE_UP)
        },
        {
          key: HOTKEY_KEYS.TABLE_DOWN,
          description: HOTKEY_DESCRIPTIONS[HOTKEY_KEYS.TABLE_DOWN],
          category: 'table',
          visual: formatHotkeyVisual(HOTKEY_KEYS.TABLE_DOWN)
        }
      );
    }

    // Action hotkeys (only for roles that can perform actions)
    if (['admin', 'super-admin', 'teacher'].includes(role || '')) {
      hotkeys.push(
        {
          key: HOTKEY_KEYS.CREATE_NEW,
          description: HOTKEY_DESCRIPTIONS[HOTKEY_KEYS.CREATE_NEW],
          category: 'actions',
          visual: formatHotkeyVisual(HOTKEY_KEYS.CREATE_NEW)
        },
        {
          key: HOTKEY_KEYS.EDIT_ITEM,
          description: HOTKEY_DESCRIPTIONS[HOTKEY_KEYS.EDIT_ITEM],
          category: 'actions',
          visual: formatHotkeyVisual(HOTKEY_KEYS.EDIT_ITEM)
        },
        {
          key: HOTKEY_KEYS.DELETE_ITEM,
          description: HOTKEY_DESCRIPTIONS[HOTKEY_KEYS.DELETE_ITEM],
          category: 'actions',
          visual: formatHotkeyVisual(HOTKEY_KEYS.DELETE_ITEM)
        }
      );
    }
