...
export const useGlobalHotkeys = () => {
  ...
  useHotkeys(HOTKEY_KEYS.SIDEBAR_TOGGLE, toggleSidebar, {
    enabled: isHotkeyEnabled && !isHelpOpen && !isCommandPaletteOpen,
    preventDefault: true,
  });
  ...
  useEffect(() => {
    ...
    const currentIndex = navItems.findIndex(item => currentPath.endsWith(item.href));
    if (currentIndex !== -1) {
      setCurrentSidebarIndex(currentIndex);
    }
  }, [navItems, setCurrentSidebarIndex]);
