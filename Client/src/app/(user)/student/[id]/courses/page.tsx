  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-background via-background to-accent/20 overflow-hidden">
      ...
      <div className="relative z-10 flex-1 p-6">
        ...
        {error && (
          <div className="glass-effect text-center text-red-500 p-6 rounded-2xl border border-red-500/20 bg-red-500/10">
            Failed to load courses.
          </div>
        )}

        {!isLoading && data?.courses?.length === 0 && (
          <div className="glass-effect text-center text-muted-foreground p-8 rounded-2xl border border-border/50">
            <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {data?.courses?.map((course) => (
            <Card key={course._id} className="glass-effect hover:shadow-2xl hover:shadow-primary/10 transition-all duration-300 cursor-pointer border-border/50 shadow-lg group rounded-2xl hover:scale-[1.02] active:scale-[0.98] relative overflow-hidden">
